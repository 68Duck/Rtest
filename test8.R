...
for (i in 1:nrow(small_countries)) {
    ...
    #NEED TO ADD CHECK WHETHER THERE ARE ANY RECTS LEFT
    for (i in 1:length(rectangles_centroids)) {
      
      if (i <= length(rectangles_centroids) && !st_is_empty(rectangles_centroids[i])) {
        rect_point <- st_coordinates(st_geometry(rectangles_centroids[i]))
    ...
    rectangles_sf <- rectangles_sf[-closest_rectangle_index]
